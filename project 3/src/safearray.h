...
public:
    ...
    // TODO: не забудьте про конструктор копирования и еще кое-что!(перегрузить операцию копирования)
    ...
    SafeArray& operator =(const SafeArray& rhvSafeArray);
...
protected:
    ...
    /// Вспомогательный метод для реализации операции копированя.
    /// Метод производит обмен элементов и значений числа элементов двух SafeArray друг с другом.
    /// Параметры метода - ссылки типа SafeArray
    static void swapArrays(SafeArray& left, SafeArray& right);
