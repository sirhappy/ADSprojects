...
void BaseBTree::insert(const Byte * k)
{
    ...
    if (root.getKeysNum() == getMaxKeys())
    {
        PageWrapper newRoot(this);

        //создаем новый корень и делаем его корнем дерева
        newRoot.allocNewRootPage();
        newRoot.setAsRoot();     
       // newRoot.setLeaf(false);
       // root.setLeaf(true);

        //прикремпляем к новому корню старый в качестве потомка и делим его
        newRoot.setCursor(0, root.getPageNum());
        newRoot.splitChild(0);
        newRoot.insertNonFull(k);   //вставляем новый ключ
    }
    else
        root.insertNonFull(k);      //просто вставляем ключ
}
...
{
    int counter = 0;    
    for (int i = 1; i <= getLastPageNum(); ++i) //проходим по всем страницам
    {
        PageWrapper pw(this);
        pw.readPage(i);
        for (int j = 0; j < pw.getKeysNum(); ++j)   //в каждой странице проходим по всем ключам
            //если ключи равны записываем найденный ключ в список и увеличиваем счетчик на 1
            if (_comparator->isEqual(k, pw.getKey(j), _recSize))    
            {      
                Byte* result = new Byte[_recSize];
                pw.copyKey(result, pw.getKey(j));
...
void BaseBTree::PageWrapper::splitChild(UShort iChild)
{
    ...
    PageWrapper childPage(this->_tree);     //дочерняя страница
    PageWrapper splittedPage(this->_tree);  //отделенная от дочерней страница 

    splittedPage.allocPage(_tree->getMinKeys(), false);
    childPage.readPageFromChild(*this, iChild);
    splittedPage.setLeaf(childPage.isLeaf());    

    //сохраняем все ключи из childPage в splittedPage, начиная с ключа с индексом _order
    for (int j = 0; j < _tree->getMinKeys(); ++j)    
        copyKey(splittedPage.getKey(j), childPage.getKey(j + _tree->getMinKeys() + 1));

    //если childPage не является листовой, то сохраняем и курсоры из нее в splittedPage,
    //начиная с курсора с индексом _order
    if (!childPage.isLeaf())    
        for (int j = 0; j <= _tree->getMinKeys(); ++j)
            splittedPage.setCursor(j, childPage.getCursor(j + _tree->getMinKeys() + 1));           
        
    setKeyNum(getKeysNum() + 1);    //добавляем место для еще одного ключа в родительской странице
    //сдвигаем курсоры в родительской страницы после курсора с номером iChild на одну позицию вправо
    for (int j = getKeysNum() - 1; j >= iChild + 1 ; --j)
        setCursor(j + 1, getCursor(j));
    
    setCursor(iChild + 1,splittedPage.getPageNum());    //добавляем курсор на отделенную страницу в позицию iChild + 1    

    //сдвигаем ключи в родительской странице начиная с iChild-ого вправо на одну позицию
    for (int j = getKeysNum() - 2; j >= iChild; --j)
        copyKey(getKey(j + 1), getKey(j));

    copyKey(getKey(iChild),childPage.getKey(_tree->getMinKeys()));  //записываем медиану в родительскую страницу    
    childPage.setKeyNum(_tree->getMinKeys());      
    
    writePage();
    splittedPage.writePage();
    childPage.writePage();
}
...
void BaseBTree::PageWrapper::insertNonFull(const Byte* k)
{
    ...
    int insertIndex = getKeysNum() - 1; //индекс определяющий позицию вставки ключа

    if (isLeaf())
    {
        setKeyNum(getKeysNum() + 1);   

        //ищем позицию вставляемого ключа
        while (insertIndex >= 0 && c->compare(k, getKey(insertIndex), _tree->getRecSize()))
        {
            copyKey(getKey(insertIndex + 1), getKey(insertIndex));  //сдвигаем очередной ключ вправо
            --insertIndex;
        }
        copyKey(getKey(insertIndex + 1), k);    //вставляем ключ в найденную позицию        
        writePage();
    }
    else
    {
        //ищем индекс курсора на страницу, в которую будет вставлен элемент
        while (insertIndex >= 0 && c->compare(k, getKey(insertIndex), _tree->getRecSize()))
            --insertIndex;
        ++insertIndex;

        PageWrapper child(this->_tree);
        child.readPageFromChild(*this, insertIndex);

        //если найденная страница заполнена, делим ее и определяем, в какую из двух получившихся будет вставлен элемент
        if (child.getKeysNum() == _tree->getMaxKeys())
        {
            splitChild(insertIndex);
            if (c->compare(getKey(insertIndex), k, _tree->getRecSize()))
                ++insertIndex;
        }
        
        child.readPageFromChild(*this, insertIndex);
        child.insertNonFull(k); //вставляем элемент в найденную страницу
    }
}
...
{
    int i = 0;    
    while (i < getKeysNum() && _tree->getComparator()->compare(getKey(i), k, _tree->getRecSize()))
        ++i;

    if (i < getKeysNum() && _tree->getComparator()->isEqual(k, getKey(i), _tree->getRecSize()))
    {
        Byte* result = new Byte[_tree->_recSize];
        copyKey(result, getKey(i));
        return result;
    }
    else if (isLeaf())
        return nullptr;
