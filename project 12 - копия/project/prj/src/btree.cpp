...
{
    PageWrapper rootPage(this);
    rootPage.readPage(getRootPageNum());

    if (rootPage.getKeysNum() == getMaxKeys())  //Корень заполнен
    {
        PageWrapper newPage(this); 
        newPage.allocNewRootPage();  //Создаем новый узел
        newPage.setAsRoot();         //Делаем его корнем
        newPage.setCursor(0, rootPage.getPageNum());  //Бывший корень становится потомком нового узла
        newPage.splitChild(0);       //Сплитим бывший корень
        newPage.insertNonFull(k);    //Вставляем элемент в новый корень
    }
    else  //В корне еще есть место
        rootPage.insertNonFull(k);
}

Byte* BaseBTree::search(const Byte* k)
{
    PageWrapper pw(this);
    pw.readPage(getRootPageNum());
    return pw.searchPW(k);
}
...
void BaseBTree::PageWrapper::splitChild(UShort iChild)
{
    ...
    PageWrapper newPage(this->_tree);    
    newPage.allocPage(_tree->getMinKeys(), false);  //новый узел

    PageWrapper splitPage(this->_tree);
    splitPage.readPageFromChild(*this, iChild);  //узел, над которым производится split

    newPage.setLeaf(splitPage.isLeaf());   
    
    //Копируем последние _order - 1 ключей из разделяемого узла в только что созданный
    copyKeys(newPage.getKey(0), splitPage.getKey(_tree->getMinKeys() + 1),_tree->getMinKeys());

    //Если разделяемый узел не является листом, то копируем и _order последних курсоров
    //из него в созданный узел   
    if(!splitPage.isLeaf())
        copyCursors(newPage.getCursorPtr(0), splitPage.getCursorPtr(_tree->getMinKeys() + 1), _tree->getMinKeys() + 1);                 
            
    setKeyNum(getKeysNum() + 1);  //Увеличиваем число ключей в родительском узле на 1    
    if (getKeysNum() - 1 > iChild)  //Проверяем нужно ли сдвигать ключи и курсоры вправо (это не нужно делать, если iChild - последний курсор в ноде)
    {
        //Сдвигаем все ключи в родительском узле вправо, начиная с ключа, левый курсор которого имеет индекс iChild
        copyKeys(getKey(iChild + 1), getKey(iChild), getKeysNum() - iChild - 1);
        //Сдвигаем все курсоры в родительском узле вправо, начиная с курсора, следующего за курсором с номером iChild
        copyCursors(getCursorPtr(iChild + 2), getCursorPtr(iChild + 1), getKeysNum() - iChild - 1);
    } 

    copyKey(getKey(iChild), splitPage.getKey(_tree->getMinKeys()));  //Записываем медиану в родителя
    setCursor(iChild + 1,newPage.getPageNum());  //Записываем курсор на newPage в родителя

    splitPage.setKeyNum(_tree->getMinKeys());   //Уменьшаем число ключей в splitPage, т.к. медиана уже была записана в родителя    
    
    //Записываем изменные узлы в файл
    writePage();
    newPage.writePage();
    splitPage.writePage();    
}
...
void BaseBTree::PageWrapper::insertNonFull(const Byte* k)
{
    ...
    if (isLeaf())  //Если узел явялется листом, вставляем ключ
    {
        setKeyNum(getKeysNum() + 1);
        //Двигаемся по ключам влево, пока не встретим ключ меньше вставляемого или ключи не закончатся
        while (i >= 0 && c->compare(k, getKey(i), _tree->getRecSize() * sizeof(Byte)))                   
            --i;

        //Сдвигаем ключи, если ключ, после которого будет производиться вставка не последний
        if(i < getKeysNum() - 2)
            copyKeys(getKey(i + 2), getKey(i + 1), getKeysNum() - i - 2);
        copyKey(getKey(i + 1), k);  //Вставляем переданный ключ

        writePage();  //Сохраняем изменненный узел
    }
    else
    {
        //Ищем индекс курсора, по которому перейдем к следующему узлу
        while (i >= 0 && c->compare(k, getKey(i), _tree->getRecSize() * sizeof(Byte)))
            --i;
        ++i;

        PageWrapper next(this->_tree);
        next.readPageFromChild(*this, i);  //Загружаем потомка по найденному индексу

        //Если потомок уже заполнен, сплитим его
        if (next.getKeysNum() == _tree->getMaxKeys())
        {
            splitChild(i);
            //Решаем, в какой из двух получившихся узлов вставлять элемент
            if (c->compare(getKey(i), k, _tree->getRecSize() * sizeof(Byte)))
                ++i;
        }
        //Загружаем следующий элемент
        next.readPageFromChild(*this, i);
        next.insertNonFull(k);
    }
}
...
Byte * BaseBTree::PageWrapper::searchPW(const Byte * k)
{
    int i = 0;

    while (i < getKeysNum() && _tree->getComparator()->compare(getKey(i), k, _tree->getRecSize() * sizeof(Byte)))
        ++i;

    if (i < getKeysNum() && _tree->getComparator()->isEqual(k, getKey(i), _tree->getRecSize() * sizeof(Byte)))
        return getKey(i);
    else if (isLeaf())
        return nullptr;
    else
    {
        this->readPageFromChild(*this, i);
        return searchPW(k);
    }
}
